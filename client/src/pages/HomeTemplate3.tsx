      ...
      <section className="py-16 md:py-24 bg-background">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
              Flexible service options that scale with your business, from growing companies to multi-location enterprises.
            </p>
          </div>
            ...
            {pricingTiers.map((tier, index) => (
              <Card
                ...
                className={`relative hover-elevate transition-all ${
                  tier.featured ? "border-[#97CC06] border-2 shadow-lg" : ""
                }`}
                data-testid={`card-pricing-${index}`}
              >
                {tier.featured && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                    <Badge className="bg-[#97CC06] text-black">Most Popular</Badge>
                  </div>
                )}
                <CardHeader>
                  ...
                  <ul className="space-y-3">
                    {tier.features.map((feature, featureIndex) => (
                      <li key={featureIndex} className="flex items-start gap-3">
                        <CheckCircle2 className="h-5 w-5 text-[#97CC06] shrink-0 mt-0.5" />
